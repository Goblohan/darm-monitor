...
def main():
    ...
    semantic_dependence = semantic_dependence_experiment()
    semantic_ablation = semantic_ablation_experiment()

    ablation_levels = semantic_ablation["candidate_levels"]

    semantic_ablation["summary"] = {
        "S1_reduces_divergence": (
            ablation_levels["S1"]["behaviorally_divergent_equivalent_pairs"]
            < ablation_levels["S0"]["behaviorally_divergent_equivalent_pairs"]
        ),
        "S2a_prime_eliminates_divergence": (
            ablation_levels["S2a_prime"][
                "behaviorally_divergent_equivalent_pairs"
            ]
            == 0
        ),
        "S2a_prime_matches_S2a_partition": (
            ablation_levels["S2a_prime"][
                "semantic_equivalence_classes"
            ]
            == ablation_levels["S2a"]["semantic_equivalence_classes"]
            and ablation_levels["S2a_prime"]["equivalent_pairs"]
            == ablation_levels["S2a"]["equivalent_pairs"]
        ),
        "S2b_adds_partition_information": (
            ablation_levels["S2b"]["semantic_equivalence_classes"]
            != ablation_levels["S2a"]["semantic_equivalence_classes"]
            or ablation_levels["S2b"]["equivalent_pairs"]
            != ablation_levels["S2a"]["equivalent_pairs"]
        ),
        "S2c_adds_partition_information": (
            ablation_levels["S2c"]["semantic_equivalence_classes"]
            != ablation_levels["S2a"]["semantic_equivalence_classes"]
            or ablation_levels["S2c"]["equivalent_pairs"]
            != ablation_levels["S2a"]["equivalent_pairs"]
        ),
    }

    payload["version"] = "v0.11"
    payload["experiment"] = (
        "causal_dependency_coverage_stress_test_plus_"
        "history_divergence_plus_representation_adequacy_plus_"
        "semantic_dependence_plus_semantic_ablation"
    )
    payload["representation_adequacy"] = adequacy
    payload["semantic_dependence"] = semantic_dependence
    payload["semantic_ablation"] = semantic_ablation
    ...
    print()
    print("DARM EMPIRICAL LAB v0.11")
    print("Experiment: semantic-dependence characterization")
    print()
    print(
        "Effective history states:",
        semantic_dependence["effective_history_states"],
    )

    for level in ("S0", "S1", "S2"):
        result = semantic_dependence["candidate_levels"][level]

        print(
            f"{level} behaviorally adequate:",
            result["behaviorally_adequate"],
        )
        print(
            f"{level} behaviorally divergent equivalent pairs:",
            result["behaviorally_divergent_equivalent_pairs"],
        )

    print()
    print("S0 fails:", semantic_dependence["S0_fails"])
    print("S1 fails:", semantic_dependence["S1_fails"])
    print("S2 repairs:", semantic_dependence["S2_repairs"])

    print()
    print("DARM EMPIRICAL LAB v0.11")
    print("Experiment: semantic-dependence ablation")
    print()
    ...
    for level in (
        ...
        result = semantic_ablation["candidate_levels"][level]
    ...
    print(
        "S2a' repairs:",
        semantic_ablation["candidate_levels"]["S2a_prime"]["behaviorally_adequate"],
    )
    print(
        "S2a repairs:",
        semantic_ablation["candidate_levels"]["S2a"]["behaviorally_adequate"],
    )
    print(
        "S2b repairs:",
        semantic_ablation["candidate_levels"]["S2b"]["behaviorally_adequate"],
    )
    print(
        "S2c repairs:",
        semantic_ablation["candidate_levels"]["S2c"]["behaviorally_adequate"],
    )
    print(
        "S2 repairs:",
        semantic_ablation["candidate_levels"]["S2"]["behaviorally_adequate"],
    )
