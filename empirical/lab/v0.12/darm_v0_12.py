...
def counterfactual_effective_history_transition(
    state: EffectiveHistoryState,
    event: Event,
) -> EffectiveHistoryState:
    """
    Counterfactual transition that neutralizes the latent capability
    change induced by activate_latent_route while preserving the
    represented-state transition.

    This is the intervention operator for v0.12. It changes only the
    effective latent capability consequence of the activation event.
    """
    after = effective_history_transition(state, event)

    if (
        event.name == "activate_latent_route"
        and not state.latent_route
    ):
        return EffectiveHistoryState(
            represented=after.represented,
            latent_route=state.latent_route,
        )
...
def counterfactual_latent_route_experiment(
    histories: list[tuple[Event, ...]],
) -> list[dict]:
    """
    Run the bounded latent-capability counterfactual experiment.

    For each admissible history containing activate_latent_route, construct
    the actual terminal state and an intervened terminal state by replaying
    the same history under the counterfactual transition relation.

    The future guarantee behavior of both terminal states is then evaluated
    independently by effective_future_signature().
    """
    results = []

    for history in histories:
        actual = EffectiveHistoryState(
            represented=INITIAL_STATE,
            latent_route=False,
        )
        counterfactual = EffectiveHistoryState(
            represented=INITIAL_STATE,
            latent_route=False,
        )
        ...
        for event in history:
            if not effective_history_authorized(actual, event):
                valid = False
                break

            if not effective_history_authorized(counterfactual, event):
                valid = False
                break

            actual = effective_history_transition(actual, event)
            counterfactual = counterfactual_effective_history_transition(
                counterfactual,
                event,
            )
        ...
        if not actual.latent_route:
            continue

        actual_behavior = effective_future_signature(actual)
        counterfactual_behavior = effective_future_signature(counterfactual)

        results.append(
            {
                "history": tuple(event.name for event in history),
                "represented_state_same": (
                    actual.represented == counterfactual.represented
                ),
                "actual_capability": effective_transfer_capability(actual),
                "counterfactual_capability": (
                    effective_transfer_capability(counterfactual)
                ),
                "capability_differs": (
                    effective_transfer_capability(actual)
                    != effective_transfer_capability(counterfactual)
                ),
                "actual_future_behavior": actual_behavior,
                "counterfactual_future_behavior": counterfactual_behavior,
                "behavior_differs": (
                    actual_behavior != counterfactual_behavior
                ),
            }
        )
...
# ---------------------------------------------------------------------------
# v0.11 semantic-dependence characterization experiment
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# v0.11 semantic-dependence characterization
# ---------------------------------------------------------------------------

SEMANTIC_ONE_STEP_DEPTH = 1
SEMANTIC_FUTURE_DEPTH = REPRESENTATION_FUTURE_DEPTH


def semantic_current_capability_signature(
    state: EffectiveHistoryState,
):
    """
    S1: current guarantee-relevant effective capability.

    This deliberately excludes implementation state such as latent_route.
    """
    return (
        effective_transfer_capability(state),
    )
...
def semantic_one_step_profile(
    state: EffectiveHistoryState,
):
    """
    S2: guarantee-relevant capability profile after each admissible
    one-step effective transition.

    The profile records the semantic capability available at the resulting
    state, not the implementation variable that caused that capability.
    """
    profile = set()
    ...
    for name in EVENTS:
        ...
        profile.add(
            (
                name,
                effective_transfer_capability(after),
                after.represented.transferred,
                violating(after.represented),
            )
        )
...
def semantic_dependence_signature(
    state: EffectiveHistoryState,
    level: str,
):
    """
    Candidate semantic summaries.

    S0: represented DARM state.
    S1: current guarantee-relevant effective capability.
    S2: one-step semantic successor profile.

    S3 is intentionally not included here. The bounded future behavior
    signature is the independent behavioral oracle, not a candidate
    representation.
    """
    represented = state.represented
    ...
    if level == "S0":
        return ("S0", state_signature)

    if level == "S1":
        return (
            "S1",
            state_signature,
            semantic_current_capability_signature(state),
        )
    ...
    if level == "S2":
        return (
            "S2",
            state_signature,
            semantic_one_step_profile(state),
        )

    raise ValueError(f"unknown semantic level: {level}")


def pairwise_semantic_dependence(
    histories,
):
    """
    Analyze every representation-equivalent pair and classify whether
    its bounded guarantee behavior diverges.

    The future behavior is computed independently of S0-S2. Therefore
    S3 cannot trivially make itself adequate.
    """
    levels = ("S0", "S1", "S2")
    ...
    for state, history in histories.items():
        for level in levels:
            key = semantic_dependence_signature(state, level)
            groups[level].setdefault(key, []).append((state, history))

    total_behaviorally_divergent_pairs = 0
    results = {}
    ...
    for level in levels:
        ...
        for members in groups[level].values():
            if len(members) < 2:
                continue
            ...
            for left_index in range(len(members)):
                for right_index in range(left_index + 1, len(members)):
                    ...
                    divergent = left_future != right_future

                    if divergent:
                        divergent_pairs += 1

                        if level == "S0":
                            total_behaviorally_divergent_pairs += 1
                        ...
                        if witness is None:
                            witness = {
                                ...
                                "signature_A": [
                                    ...
                                    for item in semantic_dependence_signature(
                                        left_state,
                                        level,
                                    )
                                ],
                                ...
                                "current_capability_A":
                                    effective_transfer_capability(left_state),
                                "current_capability_B":
                                    effective_transfer_capability(right_state),
                                "one_step_profile_A": [
                                    list(item)
                                    for item in sorted(
                                        semantic_one_step_profile(left_state),
                                        key=str,
                                    )
                                ],
                                "one_step_profile_B": [
                                    list(item)
                                    for item in sorted(
                                        semantic_one_step_profile(right_state),
                                        key=str,
                                    )
                                ],
                                "future_signature_A": [
        ...
        results[level] = {
            "history_states": len(histories),
            "semantic_equivalence_classes": len(groups[level]),
            "equivalent_pairs": equivalent_pairs,
            "behaviorally_divergent_equivalent_pairs": divergent_pairs,
            "behaviorally_adequate": divergent_pairs == 0,
            "witness": witness,
        }

    # Count the independent oracle divergence set once, from S0 pairs.
    results["oracle"] = {
        "behaviorally_divergent_pairs": (
            results["S0"]["behaviorally_divergent_equivalent_pairs"]
        ),
        "future_depth": SEMANTIC_FUTURE_DEPTH,
    }

    return results


def semantic_dependence_experiment():
    histories = enumerate_effective_histories()

    analysis = pairwise_semantic_dependence(histories)

    return {
        "history_depth": REPRESENTATION_HISTORY_DEPTH,
        "future_depth": SEMANTIC_FUTURE_DEPTH,
        "effective_history_states": len(histories),
        "candidate_levels": analysis,
        "S0_fails": not analysis["S0"]["behaviorally_adequate"],
        "S1_fails": not analysis["S1"]["behaviorally_adequate"],
        "S2_repairs": analysis["S2"]["behaviorally_adequate"],
    }



# ---------------------------------------------------------------------------
# v0.11 semantic-dependence ablation
# ---------------------------------------------------------------------------

def semantic_ablation_signature(
    state: EffectiveHistoryState,
    level: str,
):
    """
    Ablation candidates for identifying the smallest semantic information
    needed to preserve bounded guarantee-relevant future behavior.

    S0  represented DARM state
    S1  current guarantee-relevant capability
    S2a one-step event -> resulting capability
    S2b one-step event -> resulting capability + violation
    S2c one-step event -> resulting capability + transferred
    S2  full one-step semantic successor profile

    The bounded future behavior remains an independent oracle.
    """
    represented = state.represented
    ...
    capability_profile = set()
    capability_change_profile = set()
    capability_violation_profile = set()
    capability_transfer_profile = set()

    current_capability = effective_transfer_capability(state)
    ...
    for name in EVENTS:
        ...
        after = effective_history_transition(state, event)
        capability = effective_transfer_capability(after)

        capability_profile.add((name, capability))

        if capability != current_capability:
            capability_change_profile.add((name, capability))

        capability_violation_profile.add(
            (
                name,
                capability,
                violating(after.represented),
        ...
        capability_transfer_profile.add(
            (
                name,
                capability,
                after.represented.transferred,
    ...
    if level == "S2a_prime":
        return (
            "S2a_prime",
            state_signature,
            frozenset(capability_change_profile),
        )
    ...
    if level == "S2b":
        return (
            "S2b",
            state_signature,
            frozenset(capability_violation_profile),
        )
    ...
    if level == "S2c":
        return (
            "S2c",
            state_signature,
            frozenset(capability_transfer_profile),
        )
...
def json_safe(value):
    """
    Recursively convert tuple/set/frozenset structures into JSON-safe lists.
    """
...
def semantic_ablation_experiment():
    histories = enumerate_effective_histories()
    ...
    for state, history in histories.items():
        for level in levels:
            key = semantic_ablation_signature(state, level)
            groups[level].setdefault(key, []).append((state, history))
                ...
                for right_index in range(
                    ...
                    if witness is None:
                        witness = {
                            ...
                            "history_A": list(left_history),
                            "history_B": list(right_history),
                            "signature_A": json_safe(
                                semantic_ablation_signature(
                                    left_state,
                                    level,
                                )
                            ),
                            "signature_B": json_safe(
                                semantic_ablation_signature(
                                    right_state,
                                    level,
                                )
                            ...
                            "current_capability_B":
                                effective_transfer_capability(right_state),
                            "one_step_profile_A": json_safe(
                                semantic_one_step_profile(left_state)
                            ),
                            "one_step_profile_B": json_safe(
                                semantic_one_step_profile(right_state)
                            ),
                            "future_signature_A": json_safe(left_future),
                            "future_signature_B": json_safe(right_future),
                        }
    ...
    return {
        ...
        "future_depth": SEMANTIC_FUTURE_DEPTH,
        "effective_history_states": len(histories),
        "candidate_levels": results,
    }



def latent_route_counterfactual_experiment():
    """
    Characterize whether the latent-route capability distinction is
    causally associated with a difference in bounded future behavior.

    The history set is the existing bounded effective-history set.
    Actual and counterfactual histories are replayed with their respective
    transition relations, while future behavior is evaluated independently
    with effective_reachable().
    """
    histories = enumerate_effective_histories()

    history_list = [
        tuple(Event(name) for name in history)
        for history in histories.values()
    ]
    results = counterfactual_latent_route_experiment(history_list)

    capability_behavior_pairs = {
        (
            result["capability_differs"],
            result["behavior_differs"],
        )
        for result in results
    }
    ...
    capability_changed = [
        result
        for result in results
        if result["capability_differs"]
    ]

    represented_state_invariant = all(
        result["represented_state_same"]
        for result in capability_changed
    )

    behavior_changed_given_capability_change = [
        result
        for result in capability_changed
        if result["behavior_differs"]
    ]

    behavior_unchanged_given_capability_change = [
        result
        for result in capability_changed
        if not result["behavior_differs"]
    ]

    behaviorally_distinguishing = [
        result
        for result in results
        if result["capability_differs"]
        and result["behavior_differs"]
    ]
    ...
    capability_only = [
        ...
        for result in results
        if result["capability_differs"]
        and not result["behavior_differs"]
    ]

    return {
        "history_depth": REPRESENTATION_HISTORY_DEPTH,
        "effective_history_states": len(histories),
        "tested_latent_route_histories": len(results),
        "capability_behavior_pairs": sorted(
            [list(pair) for pair in capability_behavior_pairs]
        ),
        "capability_changed_cases": len(capability_changed),
        "represented_state_invariant_for_capability_changes": (
            represented_state_invariant
        ),
        "behavior_changed_given_capability_change": len(
            behavior_changed_given_capability_change
        ),
        "behavior_unchanged_given_capability_change": len(
            behavior_unchanged_given_capability_change
        ),
        "capability_and_behavior_distinguishing_cases": len(
            behaviorally_distinguishing
        ),
        "capability_only_cases": len(capability_only),
        "behaviorally_distinguishing_witnesses": json_safe(
            behaviorally_distinguishing
        ),
        "capability_only_witnesses": json_safe(
            capability_only
        ),
    }
...
def main():
    result = latent_route_counterfactual_experiment()

    path = Path(__file__).with_name("results.json")

    payload = {
        "version": "v0.12",
        "experiment": "counterfactual_semantic_dependence",
        "experiment_question": (
            "Does neutralizing a semantic capability distinction change "
            "guarantee-relevant future behavior?"
        ),
        "method": (
            "Replay the same bounded histories under the actual and "
            "counterfactual transition relations, neutralizing only the "
            "latent-route capability transition, then compare their "
            "independent future behavioral signatures."
        ),
        "result": result,
    }
    ...
    print()
    print("DARM EMPIRICAL LAB v0.12")
    print("Experiment: counterfactual semantic dependence")
    print()
    print(
        "Effective history states:",
        result["effective_history_states"],
    )
    print(
        "Tested latent-route histories:",
        result["tested_latent_route_histories"],
    )
    print(
        "Capability-changing cases:",
        result["capability_changed_cases"],
    )
    print(
        "Represented-state invariant:",
        result["represented_state_invariant_for_capability_changes"],
    )
    print(
        "Behavior changed given capability change:",
        result["behavior_changed_given_capability_change"],
    )
    print(
        "Behavior unchanged given capability change:",
        result["behavior_unchanged_given_capability_change"],
    )
    print(
        "Capability + behavior distinguishing cases:",
        result["capability_and_behavior_distinguishing_cases"],
    )
    print(
        "Capability-only cases:",
        result["capability_only_cases"],
    )
    print()
    print("Capability/behavior combinations:")

    for pair in result["capability_behavior_pairs"]:
        print(" ", pair)

    print()
    print("Behaviorally distinguishing witnesses:")

    for witness in result["behaviorally_distinguishing_witnesses"]:
        print(witness)

    print()
    print("Capability-only witnesses:")

    for witness in result["capability_only_witnesses"]:
        print(witness)
