...
theorem guarded_ratification_step_preserves_coherence
  ...
  exact DARM.Ratification.guarded_ratification_preserves_coherence
    reqs allowedCapLimit validToken s t p δ w hcoh hguard
...
def historyStepState
    ...
    (hs : HistoryState (CapId := CapId) (ActionId := ActionId)) :
    HistoryState (CapId := CapId) (ActionId := ActionId) :=
  let s' := historyStep reqs allowedCapLimit validToken policy hs.history hs.state
  { state := s'
...
def historyStateStep
    ...
    HistoryState (CapId := CapId) (ActionId := ActionId) :=
  let s' :=
    historyStep reqs allowedCapLimit validToken policy hs.history hs.state
  { state := s'
    history := hs.history.concat (observe s') }
...
def generateTrajectory
  ...
  | n + 1, hs =>
      generateTrajectory
        reqs allowedCapLimit validToken policy n
        (historyStateStep
          reqs allowedCapLimit validToken policy hs)
...
/-- **Capability confinement against an unrestricted adaptive adversary.**
    ...
    trajectory is a separate, harder result: it reqs threading the evolving
...
theorem generateTrajectory_preserves_capInvariant
    ...
    capInvariant allowedCapLimit
      (generateTrajectory reqs allowedCapLimit validToken policy n hs).state := by
  ...
  | succ k ih =>
      ...
      show capInvariant allowedCapLimit
        (historyStateStep reqs allowedCapLimit validToken policy hs).state
      simp only [historyStateStep, historyStep]
      exact step_preserves_capInvariant hs.state
